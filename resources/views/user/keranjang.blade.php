                ...
                <div class="col-12 col-lg-8">
                    ...
                    <div class="table-responsive scrollbar mx-n1 px-1">
                        <table class="table fs--1 mb-0 border-top border-200">
                            <thead>
                                <tr>
                                    <th class="white-space-nowrap align-middle ps-4 fs--1 text-dark" style="width:50;" data-sort="path_produk">IMAGE</th>
                                    <th class="sort white-space-nowrap align-middle" scope="col" style="max-width: 50px">PRODUCTS</th>
                                    ...
                                    <th class="sort align-middle text-end" scope="col" style="width:250px;">ACTION</th>
                                    <th class="sort text-end align-middle pe-0" scope="col"></th>
                                        ...
                                        <tr class="cart-table-row btn-reveal-trigger">
                                            <td class="align-middle white-space-nowrap py-0"><a
                                                ...
                                                src="{{ asset('storage/Product/' .$pesanan->produk->path_produk) }}" alt="" width="53" /></a></td>
                                                        <td class="products align-middle col-2" style="max-width: 50px;">
                                                            <a class="fw-semi-bold mb-0 text-truncate d-inline-block" href="#">
                                                                {{ ucfirst(Str::limit($pesanan->produk->nama_produk, 10, $end='...')) }}
                                                            </a>
                                                        </td>
                                            <td class="color align-middle white-space-nowrap fs--1 text-900 text-truncate" >{{ $pesanan->produk->kategori->nama_kategori  }}</td>
                                            <td class="price align-middle text-900 fs--1 fw-semi-bold text-end">Rp. {{ number_format($pesanan->produk->harga * $pesanan->jumlah, 0, ',', '.') }}</td>
                                            <td class="quantity align-middle fs-0 ps-5">
                                                <div class="input-group input-group-sm flex-nowrap"
                                                    data-quantity="data-quantity"><input
                                                        ...
                                                        type="number" min="1" value="{{ $pesanan->jumlah }}" name="jumlah[{{ $pesanan->id }}]"></div>
                                            </td>
                                            <td class="total align-middle fw-bold text-1000 text-end">Rp. {{ number_format($pesanan->produk->harga * $pesanan->jumlah, 0, ',', '.') }}</td>
                                            <td class="align-middle white-space-nowrap text-end pe-0 ps-3">
                                                <form action="{{ route('keranjang.destroy', $pesanan->id) }}" method="POST">
                                                    @csrf
                                                    @method('delete')
                                                    <button type="submit" class="btn btn-sm text-500 hover-text-600 me-2">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{ route('checkout-keranjang')  }}" method="post">
                            @method('patch')
                            @csrf
                            <button class="btn btn-primary w-100">Proceed to check out<span class="fas fa-chevron-right ms-1 fs--2"></span>
                            </button>
                        </form>
                    </div>
                </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function updateTotals() {
            ...
            totalCells.forEach(function (totalCell, index) {
                ...
                var price = parseFloat(totalCell.textContent.replace('Rp. ', '').replace('.', '').replace(',', ''));
